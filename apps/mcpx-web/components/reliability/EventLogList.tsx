    ...
    <div className="lg:col-span-6 space-y-3 flex flex-col">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">
          Durable Sequenced Event Log ({eventLog.length})
        </span>
        {eventLog.length > 0 && (
          <button
            onClick={onClearLog}
      ...
      <div className="flex-1 min-h-75 max-h-120 rounded-xl border border-slate-800 bg-slate-950 p-4 font-mono text-xs overflow-y-auto space-y-2.5">
        {eventLog.length === 0 ? (
          <div className="h-full flex items-center justify-center text-slate-600 italic">
            No events logged yet. Click [ RUN CHALLENGE SCENARIO ] to execute.
          </div>
        ) : (
          eventLog.map((ev, index) => {
            ...
            } else if (ev.type.includes("RECONCILIATION") || ev.type.includes("RECONCILING")) {
              badgeColor = "bg-indigo-950 text-indigo-300 border-indigo-500/40";
            } else if (ev.type.includes("RECOVERED") || ev.type.includes("FOUND") || ev.type.includes("COMMITTED")) {
              badgeColor = "bg-emerald-950 text-emerald-300 border-emerald-500/40";
            } else if (ev.type.includes("FAILED") || ev.type.includes("ABORT")) {
              badgeColor = "bg-rose-950 text-rose-300 border-rose-500/40";
            }

            const seqLabel = ev.sequence ? `SEQ #${ev.sequence}` : `#${index + 1}`;
